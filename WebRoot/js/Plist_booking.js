...
$(document).ready(function(){
    ...
    $(".btn_search1").on("click",function(){
        if($(".searchbybookingdate").val() ==''){
            alert("请输入预定日期！");
        }else{
        	getCountbydate();
        	
        	$("#News-Pagination").pagination(count_date, {
                items_per_page:6,//每页显示多少条数据
                current_page:0,//当前显示第几页数据
                num_display_entries:6,//分页显示的条目数
                next_text:"下一页",
                prev_text:"上一页",
                num_edge_entries:2,//链接分页主体，显示的条目数
                callback:handlePaginationClick_date
            });
        }
    });

    $(".btn_search2").on("click",function(){
        if($(".searchbyteacher").val() ==''){
            alert("请输入教师名！");
        }else{
        	getCountbyusername();
//        	alert(count_username);
        	$("#News-Pagination").pagination(count_username, {
                items_per_page:6,//每页显示多少条数据
                current_page:0,//当前显示第几页数据
                num_display_entries:6,//分页显示的条目数
                next_text:"下一页",
                prev_text:"上一页",
                num_edge_entries:2,//链接分页主体，显示的条目数
                callback:handlePaginationClick_username
            });
        }
    });
});
...
function  getCountbydate() {
	...
	$.ajax({ //使用ajax与服务器异步交互
        ...
        url:"Managerbooking?s="+new Date().getTime(), //后面加时间戳，防止IE辨认相同的url，只从缓存拿数据
        type:"POST",
        data: {act:"getSumbydate",date:date}, //$('#yourformid').serialize()；向后台发送的form表单中的数据
        dataType:"json", //接收返回的数据方式为json
...
function  getCountbyusername() {
	...
	$.ajax({ //使用ajax与服务器异步交互
        ...
        url:"Managerbooking?s="+new Date().getTime(), //后面加时间戳，防止IE辨认相同的url，只从缓存拿数据
        type:"POST",
        data: {act:"getSumbyusername",username:username}, //$('#yourformid').serialize()；向后台发送的form表单中的数据
        dataType:"json", //接收返回的数据方式为json
...
function handlePaginationClick_username(new_page_index, pagination_container) {
    // This selects 20 elements from a content array
	var username = $(".searchbyteacher").val();
	...
	$.ajax({ //使用ajax与服务器异步交互
        url:"Managerbooking?s="+new Date().getTime(), //后面加时间戳，防止IE辨认相同的url，只从缓存拿数据
        type:"POST",
        data: {act:"getAllbyusername",page:new_page_index,username:username}, //$('#yourformid').serialize()；向后台发送的form表单中的数据
        dataType:"json", //接收返回的数据方式为json
...
function handlePaginationClick_date(new_page_index, pagination_container) {
    // This selects 20 elements from a content array
	var date = $(".searchbybookingdate").val();
	$.ajax({ //使用ajax与服务器异步交互
        url:"Managerbooking?s="+new Date().getTime(), //后面加时间戳，防止IE辨认相同的url，只从缓存拿数据
        type:"POST",
        data: {act:"getAllbydate",page:new_page_index,date:date}, //$('#yourformid').serialize()；向后台发送的form表单中的数据
        dataType:"json", //接收返回的数据方式为json
