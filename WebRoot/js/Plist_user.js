...
function handlePaginationClick(new_page_index, pagination_container) {
	...
	$.ajax({ //使用ajax与服务器异步交互
        url:"Manageuser?s="+new Date().getTime(), //后面加时间戳，防止IE辨认相同的url，只从缓存拿数据
        type:"POST",
        data: {act:"getAlluser",page:new_page_index}, //$('#yourformid').serialize()；向后台发送的form表单中的数据
        dataType:"json", //接收返回的数据方式为json
...
//        	alert(data.length);
        	for (var i = 0; i < data.length; i++) {
				(function(i) {
					...
					var td1 = $("<td class='username"+i+"'>"+data[i].username+"</td>");
					var td2 = $("<td class='depart"+i+"'>"+data[i].depart+"</td>");
					var td3 = $("<td class='phone"+i+"'>"+data[i].phone+"</td>");
					var td4 = $("<td></td>");
...
//						alert(id);
						...
						$("#user_username").val($(".username"+i).text());
						$("#user_depart").val($(".depart"+i).text());
						$("#user_phone").val($(".phone"+i).text());
						...
						$("#btn_ok").on("click",function(){
						    ...
						    if (username==''){
						        ...
						        $(".warning").text("警告：系别不能为空！");
						        $("#user_depart").focus();
						    }else if (phone==''){
						    	$(".warning").text("警告：电话不能为空！");
						        $("#user_phone").focus();
						    }else{
						    	$.ajax({ //使用ajax与服务器异步交互
//						    		async:false,
