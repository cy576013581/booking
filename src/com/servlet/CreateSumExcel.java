...
public class CreateSumExcel extends HttpServlet {
...
//				System.out.println(roomid+"*"+roomname);
				...
				int titleSize[] = {12,90,90,90,90,90,90,90};
				ExportExcel pee = new ExportExcel(response,filename+"总课表");
				
				pee.setFormtitle("中原工学院信息商务学院"+filename+"总课表");
				pee.setRowheigth(2000);
...
//						System.out.println(week_Monthday);
						list = produceDate(week_Monthday,position);
					...
					for(int i=0;i<list.size();i++){
						for(int j=1;j<8;j++){
							if(titleName1[sheetindex][j].equals(list.get(i).get("classtime"))){
								...
								data[sheetindex][section-1][j] = roomname+"--"+username+",--"+classname+"--"+coursename+"--"+students+"人--"+phone;
...
//					System.out.println(list);
	...
	public List<Map<String,String>> produceDate(String strdate,int position){
...
//		            System.out.println(list.get(i));  
	...
	public List<Map<String,String>> getClass(String date,int position){
		List<Map<String,String>> list= new ArrayList<>();
		ScheduleDAO sche = new ScheduleDAO();
		try {
			list = sche.findClassbyPosition(date,position);
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
